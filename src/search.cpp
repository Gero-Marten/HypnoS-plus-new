...
void update_exploration_factor(const Position& pos, int depth, int timeLeftMs) {
    ...
    if (timeLeftMs > 60000) { // Abundant time remaining.
        new_exploration_factor += 0.05 * Search::exploration_decay_factor;
    } else if (timeLeftMs < 10000) { // Limited time remaining.
        new_exploration_factor -= 0.05 * Search::exploration_decay_factor;
    }

    // Adjust based on search depth.
    new_exploration_factor -= 0.005 * std::sqrt(depth) * Search::exploration_decay_factor;
...
Skill skill(Options["Skill Level"], Options["UCI_LimitStrength"] ? int(Options["UCI_Elo"]) : 0);
           ...
           && !(Limits.depth && mainThread && rootDepth > Limits.depth))
    {
        ...
        for (pvIdx = 0; pvIdx < multiPV && !Threads.stop; ++pvIdx)
        {
            ...
            delta     = Value(5) + int(avg) * avg / 13424;
...
Value search(Position& pos, Stack* ss, Value alpha, Value beta, Depth depth, bool cutNode) {
    ...
    {
        int bonus = std::clamp(-10 * int((ss - 1)->staticEval + ss->staticEval), -1644, 1384);
...
moves_loop:  // When in check, search starts here
    ...
    if ((tte->bound() & BOUND_LOWER) && tte->depth() >= depth - 4 && tte->value() >= probCutBeta
        && std::abs(beta) < VALUE_TB_WIN_IN_MAX_PLY
    ...
    while ((move = mp.next_move()) != Move::none())
    {
        ...
        if (ss->ply < thisThread->rootDepth * 2)
        {
            ...
            if (!rootNode && move == ttMove && !excludedMove
                ...
                && tte->depth() >= depth - 3)
            {
                Value singularBeta  = ttValue - (56 + 79 * (ss->ttPv && !PvNode)) * depth / 64;
    ...
    else if (ttMove && !allNode)
        thisThread->mainHistory[us][ttMove.from_to()] << stat_bonus(depth) * 23 / 100;
    if (PvNode)
