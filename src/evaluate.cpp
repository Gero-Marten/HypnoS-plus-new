...
// Global Variables
ShashinStyle CurrentStyle = {10, 15, 10, 10, 15, 10};  // Default Shashin style.
...
void set_shashin_style(Style style) {
    switch (style) {
        case Style::Tal: // Aggressive, tactical style.
            CurrentStyle = {25, 5, 0, 25, 3, 0};
            break;

        case Style::Capablanca: // Balanced, positional style.
            CurrentStyle = {10, 15, 10, 10, 15, 10};
            break;

        case Style::Petrosian: // Defensive, strategic style.
...
void set_shashin_style(const std::string& style) {
    if (style == "Tal") {
        ...
        // Default to Capablanca style for invalid input.
        set_shashin_style(Style::Capablanca);
    }
}
